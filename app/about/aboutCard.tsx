    ...
    <div className="relative w-full px-4 py-8">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 md:flex-row md:gap-4">
        {content.map((item, idx) => (
          <div
            key={idx}
            className="relative flex h-72 flex-col justify-end overflow-hidden rounded-2xl border border-gray-800 bg-white px-6 py-6 shadow-xl md:flex-1"
          >
            <h2 className="relative z-10 mb-3 text-xl font-bold text-neutral-800">
              {item.title}
            </h2>
            <p className="relative z-10 mb-3 text-sm text-neutral-400 h-36">
              {item.desc}
            </p>
          </div>
        ))}
