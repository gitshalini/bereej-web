...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn("w-full fixed top-0 inset-x-0 z-50", className)}
    >
    ...
    >
      {items.map((item, idx) => (
        <div
          ...
          onMouseLeave={() => setHovered(null)} // ✅ unified hover logic
        >
          <a
            href={item.link}
            onClick={onItemClick}
    ...
    <div className="flex w-full flex-col gap-2">
      {items.map((item, idx) => (
        <div key={`mobile-nav-${idx}`} className="w-full">
          ...
          >
            <a href={item.link} className="flex-1" onClick={onItemClick}>
              {item.name}
            </a>
            {item.children && (
              <button onClick={() => toggleSubmenu(idx)}>
                {openIndex === idx ? (
